...
Ниже представлена **полная шпаргалка** по основным алгоритмам и структурам данных, включая все 22 темы, а также **детальный** разбор дополнительных аспектов:
...
- Пример: Merge Sort — \(T(n)=2T(n/2)+O(n)\) → \(O(n\log n)\).
...
## 2. Оценка сложности алгоритма по памяти (подробно)
